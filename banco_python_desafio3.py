...
class ContaCorrente(Conta):
    ...
    # Conta Corrente possui limite de apenas 3 saques
    def sacar(self, valor):
        numero_saques = len(
        ...
        if excedeu_limite:
            print(f"ERRO: Valor limite de saque excedido. O limite é R$ {self._limite:.2f}")
...
def menu():
    menu = '''
    Bem vindo ao Banco Python - Versão POO!
    +++++++++++++++++++++++++++++++++++++++++++++
...
def depositar(clientes):
    ...
    if not cliente:
        print("ERRO: Correntista não encontrado!")
        return
...
def criar_conta(numero_conta, clientes, contas):
    ...
    conta = recuperar_conta_cliente(cliente)
    if conta:
        print("ERRO: Cliente já possui conta corrente cadastrada!\nÉ permitido apenas uma por cliente.")
        return
...
def listar_contas(contas):
    ...
    if not contas:
        print("No momento ainda não temos nenhuma conta cadastrada!")
...
def listar_clientes(clientes):
    ...
    if not clientes:
        print("No momento ainda não temos nenhum cliente cadastrado!")
